...
build_schema_context:
	docker run --rm --privileged ${BIND_MOUNT_ARGS} ${AWS_CREDS_BIND_MOUNT} ${PROD} ${NAME}:${VERSION} /bin/bash -c "python3 generate_schema_context.py > /root/.cache/schema.txt"

build_schema_context_dev:
	docker run --rm --privileged ${BIND_MOUNT_ARGS} ${AWS_CREDS_BIND_MOUNT} ${DEV} ${NAME_DEV}:${VERSION} /bin/bash -c "python3 generate_schema_context.py > /root/.cache/schema_dev.txt"
