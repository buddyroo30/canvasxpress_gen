...
USE_MODEL="openai" #"titan" or "openai" or "google_gemini" or "ollama"
...
def ask():
    ...
    if prompt == 'Thumbs down':
        ...
        if USE_MODEL == "titan":
            generated_text = llm.generate_results_titan(prompt,model='amazon.titan-tg1-large', max_token_count=int(max_new_tokens), topp=float(topp), temperature=float(temperature))
            ...
            generated_text = llm.generate_results_ollama(prompt)
