...
def ask():
    ...
    if prompt == 'Thumbs down':
        ...
        elif USE_MODEL == "azure_openai":
            generated_text = llm.generate_results_azure_openai(prompt, model=model, max_new_tokens=int(max_new_tokens),topp=float(topp),temperature=float(temperature),presence_penalty=float(presence_penalty),frequency_penalty=float(frequency_penalty))
...
@app.route('/ask_generic',methods=['GET','POST'])
def ask_generic():
    ...
    if utils.empty(prompt):
        errResp = { 'text': "Error: you must provide a prompt for the LLM", 'success': False}
        return(json.dumps(errResp))
    ...
    llm_time_taken = end - start
    if utils.empty(generated_text) or generated_text.strip() == "''":
        errResp = { 'text': "Error: No response was generated by the LLM, please try again.", 'success': False }
        return(json.dumps(errResp))
    print("RAW generated_text:\n" + generated_text)
