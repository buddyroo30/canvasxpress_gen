...
def ask():
    ...
    if prompt == 'Thumbs down':
        ...
        if USE_MODEL == "titan":
            ...
            generated_text = generate_results_google_gemini(prompt,model=model,temperature=float(temperature))
        ...
        configObj = {}
        try:
            #print("BEFORE: " + generated_text)
            #generated_text.removeprefix('```json')
            #generated_text.removesuffix('```')
            #print("AFTER: " + generated_text)
            configObj = json.loads(generated_text.strip())
